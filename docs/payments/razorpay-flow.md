...
3. Order creation derives line items and totals from MongoDB product records; the API converts that stored server-calculated total to integer paise and creates a Razorpay order.
...
- Reserve/decrement inventory atomically if the business requires stock to be held before payment.
